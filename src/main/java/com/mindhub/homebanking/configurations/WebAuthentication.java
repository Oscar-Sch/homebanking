    ...
    public void init(AuthenticationManagerBuilder auth) throws Exception {
        ...
        auth.userDetailsService(user-> {
            ...
            if(emailValidation(user)){
                client = clientRepository.findByEmail(user);
            }else
                if(userNameValidation(user)){
                client = clientRepository.findByUserName(user);
            }else
                if(phoneNumberValidation(user)){
                client = clientRepository.findByPhoneNumber(user);
            }
            if (client != null) {
